...
import future from "./logo/future.png";
...
const baseUrl =  process.env.NODE_ENV === "production"
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://compareplan-1.onrender.com/api/add-detail", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      {/* Main Content */}
      <div className="flex mb-10 justify-center mx-auto  bg-opacity-24   w-full max-w-6xl p-8 flex-grow bg-[#f3f4f6]">
        {/* Left panel for insurer selection */}
        <div className="w-[40%] p-4  border-r rounded-2xl bg-[#0092ff] bg-opacity-10 shadow-black/10 backdrop-blur-[9.3px] border border-white/60 shadow-[0px_1px_7px_7px_rgba(0,_0,_0,_0.1)]">
          <h2 className="text-lg font-bold mb-4">&#128073; Select Insurer</h2>
          <div
            className="grid grid-cols-3 gap-4 overflow-y-auto   rounded-2xl p-4  bg-[#e1f2ff]"
            style={{ maxHeight: "350px" }}
          >
            {insurers.map((insurer, index) => (
              <button
                key={index}
                className={`p-3 flex  bg-[#0092ff] bg-opacity-10 justify-center rounded-lg text-center font-medium ${
        ...
        {/* Right Panel for Plans */}
          ...
          {!selectedInsurer ? (
            ...
            <>
            <div className="w-1/2  p-4 bg-[#e1f2ff]">
              ...
              <div className="overflow-y-auto max-h-80  rounded p-4">
                {selectedInsurer.plans.map((plan) => (
                  <button
                    key={plan}
                    className={`w-full p-3 mb-2 rounded-lg font-semibold text-left ${
                      selectedPlans.some(
                        (p) =>
                          p.planName === plan &&
